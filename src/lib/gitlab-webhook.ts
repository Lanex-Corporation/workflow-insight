...
const GitlabWebhookHandler = (request: any) => {
      ...
      if (eventType == GitlabWeebhookEventType.MERGE_REQUEST || eventType == GitlabWeebhookEventType.PUSH) {
        ...
        if (ticket && project && user && mrStats) {
          if (eventType == GitlabWeebhookEventType.PUSH) {
            ...
            if (reviewerId && mergeRequest && !mergeRequest.blocking_discussions_resolved && authorUser) {
  ...
  function getTargetBranch() {
    ...
    // Available during comment event
    if (!branch && payload.project)
      branch = payload.project.name;
...
export const GitlabWebhook = async (req: Request, res: Response) => {
  ...
  const webhook = GitlabWebhookHandler(req);
  ...
  if (!isSuccess) {
    ...
    return res.status(200).send("Unable to process the event — it may have been intentionally ignored or an error occurred.");
  }

  console.log("Event Processed: Pull request opened event recorded.");
  return res.status(200).send("Pull request opened event recorded.");
};